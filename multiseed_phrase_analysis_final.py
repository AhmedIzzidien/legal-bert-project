...
"""
===============================================================================
MULTI-SEED PHRASE DIFFERENTIAL ANALYSIS (FINAL)
===============================================================================
...
Outputs:
    ...
    - Topic-controlled consistency analysis with FDR correction
...
# Load topics/text data (all cases)
print(f"\n📂 Loading: {TOPICS_FILE}")
...
if len(df) < len(df_consistency) * 0.95:
    print(f"\n❌ ERROR: Too many rows lost in merge!")
    exit(1)
...
wrong_cases = df[df['always_wrong']].copy()
right_cases = df[df['always_right']].copy()
...
# PRECOMPUTE document-term matrix for ALL cases
...
wrong_indices = df[df['always_wrong']].index.tolist()
right_indices = df[df['always_right']].index.tolist()
...
for topic in valid_topics:
    ...
    topic_wrong_mask = topic_mask & df['always_wrong']
    topic_right_mask = topic_mask & df['always_right']
    
    # Get positional indices
    topic_wrong_pos = [idx_to_pos[idx] for idx in df[topic_wrong_mask].index]
    topic_right_pos = [idx_to_pos[idx] for idx in df[topic_right_mask].index]
...
print("\n" + "-"*70)
print("🔴 TOP PHRASES OVERREPRESENTED IN ALWAYS-WRONG CASES (0/5)")
print("-"*70)
...
for _, row in error_phrases.iterrows():
    ...
    cons = f"{row['consistency']*100:.0f}%" if row['consistency'] > 0 else '—'
...
print("\n" + "-"*70)
print("🟢 TOP PHRASES OVERREPRESENTED IN ALWAYS-RIGHT CASES (5/5)")
print("-"*70)
...
if len(cts) > 0:
    summary_data.append({
        ...
        'N_FDR_Significant': int(cts['significant_fdr'].sum())
    })
...
if len(tpe) > 0:
    ...
    summary_data.append({
        ...
        'N_FDR_Significant': int(tpe['significant_fdr'].sum())
    })
...
if len(rs) > 0:
    summary_data.append({
        ...
        'N_FDR_Significant': int(rs['significant_fdr'].sum())
    })
...
if len(tpc) > 0:
    ...
    summary_data.append({
        ...
        'N_FDR_Significant': int(tpc['significant_fdr'].sum())
    })

summary_df = pd.DataFrame(summary_data)
summary_df.to_csv(f"{OUTPUT_DIR}/phrase_summary_table.csv", index=False)
print(f"   ✅ phrase_summary_table.csv")
...
ax.scatter(plot_df['delta'] * 100, plot_df['consistency'] * 100, c=colors, alpha=0.6, s=50)
...
for _, row in plot_df[plot_df['is_cross_topic_signal'] == True].iterrows():
    ax.annotate(row['phrase'], (row['delta']*100, row['consistency']*100), fontsize=9, ha='left')

for _, row in plot_df[plot_df['is_reliable_signal'] == True].head(5).iterrows():
    ax.annotate(row['phrase'], (row['delta']*100, row['consistency']*100), fontsize=9, ha='right')
...
print(f"   - Always-wrong (0/5): {n_always_wrong} cases")
...
if len(cross_topic_errors) > 0:
    for _, row in cross_topic_errors.head(5).iterrows():
        print(f"      '{row['phrase']}': Δ=+{row['delta']:.3f}")
